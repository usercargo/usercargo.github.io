...
const KEEP_PERCENTAGE = 1;// 0.15; // keep only 15% of audio file

//let happppy=document.querySelector("#happppy");

let essentia = null;
//let essentiaAnalysis;
let featureExtractionWorker = null;
let inferenceWorkers = {};
// const modelNames = ['mood_happy', 'mood_sad', 'mood_relaxed', 'mood_aggressive', 'danceability'];// ['mood_happy'];
const modelNames = ['mood_happy', 'mood_sad'];// ['mood_happy'];
let inferenceResultPromises = [];
let moodCallbackResult;
...
window.decodeFile = function (arrayBuffer, callback) {
    moodCallbackResult = callback;
    console.log("TP2 ", arrayBuffer, audioCtx);
...
function createFeatureExtractionWorker() {
    featureExtractionWorker = new Worker('./cdn/mood/featureExtraction.js');

    featureExtractionWorker.onmessage = function listenToFeatureExtractionWorker(msg) {
        console.log("TP3 ", featureExtractionWorker);
        // feed to models
...
function createInferenceWorkers() {
    modelNames.forEach((n) => {
        inferenceWorkers[n] = new Worker('./cdn/mood/inference.js');
        inferenceWorkers[n].postMessage({
            name: n
        });
...
function collectPredictions() {
    if (inferenceResultPromises.length == modelNames.length) {
        Promise.all(inferenceResultPromises).then((predictions) => {
            ...
            console.log("RESULT : ", allPredictions);
            if (moodCallbackResult) {
                window[moodCallbackResult](allPredictions);
            }
...
window.initMoodAnalyse = function () {
